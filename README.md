...
*   **성경 본문 해석기 (`BibleScriptureResolver`)**: 복잡한 인용구를 분석하고 다국어 본문을 매칭하는 강력한 핵심 엔진을 탑재했습니다.
*   **AI 자동 파싱**: `Gemini 2.0 Flash`가 이미지를 분석하여 날짜별 JSON 데이터를 자동 생성합니다. (중앙 집중식 약어 관리로 정확도 향상)
*   **다국어 지원**: 한국어(개역한글), 영어(ESV/NLT), 몽골어(MUV) 지원.
