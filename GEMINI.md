...
2. **데이터 파싱 및 발송 규칙**:
    ...
    - **본문 발송**: 구약과 신약은 길이가 길어 진도표(요약)에만 표시하고, 실제 텔레그램 본문은 **QT, 시편, 잠언**만 전송합니다.
    - **상속 로직**: AI가 인식하지 못한 권수(빈칸)는 이전 날짜의 권수를 자동으로 상속받도록 `gemini_parser.py`가 처리합니다.
3. **스마트 실행**: 모든 실행은 `python main.py`를 권장하며, 데이터 누락 시 자동으로 AI 파싱이 수행되도록 설계되었습니다.
4. **파일명 규칙**: `assets/`에 이미지를 업로드할 때는 반드시 `{연도}년_{월}월_{구분}_passage` 형식을 준수하십시오.
